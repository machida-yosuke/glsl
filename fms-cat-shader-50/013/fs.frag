...
float distFunc( vec3 _p ) {
    ...
    for ( int i = 0; i < 6; i ++ ) {
        float ii = float( i );
        vec3 pos = vec3(
            sin( time * 3.0 * sin( ii * 5.2 ) + 60.0 * sin( ii * 4.2 ) ),
            sin( time * 3.0 * sin( ii * 1.6 ) + 60.0 * sin( ii * 5.1 ) ),
            sin( time * 3.0 * sin( ii * 5.4 ) + 60.0 * sin( ii * 5.6 ) )
        ) * 0.3;
        dist = smin( distFuncSphere( p - pos, 0.2 ), dist, 0.3 );
    }
    return dist;
