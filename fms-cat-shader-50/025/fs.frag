...
// アニメーションするランダム
float aniRandom( float _phase ) {
    float v0 = random( floor( _phase ) );
    float v1 = random( floor( _phase ) + 1.0 );
    return mix( v1, v0, exp( -fract( _phase ) * 6.0 ) );
}
...
float distFunc( vec3 _p ) {
    vec3 p = _p;
    float inside = distFuncBox( p, vec3( 1.5, 0.3, 1.5 ) );
    p = ifs(
        p,
        0.1 * vec3( aniRandom( time + 10.0 ), aniRandom( time + 21.0 ), aniRandom( time + 31.0 ) ),
        1.0 + 1.0 * vec3( aniRandom( time ), aniRandom( time + 40.0 ), aniRandom( time + 80.0 ) )
    );
    float dist = distFuncBox( p, vec3( 0.1 ) );
    dist = max( dist, -inside );
    return dist;
...
void main() {
    ...
    if ( rayDist < 1E-2 ) { // もし物体に衝突したなら
        ...
        float edge = saturate( length( normalFunc( rayPos, 1E-4 ) - normalFunc( rayPos, 4E-3 ) ) );
        ...
        // 色を足し掛けする
        col += dif * mtlCol * ( 1.0 - mtlReflective );
        col += vec3( 1.0 ) * spe * 0.5;
        col += edge * vec3( 1.0, 0.1, 0.4 );
