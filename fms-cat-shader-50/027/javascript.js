...
let prg = gl3.program.create_from_source(
    ...
    [ 'position' ],
    [ 3 ],
    [ 'resolution', 'mouse', 'time', 'isVert', 'textureLenna' ],
    [ '2fv', '2fv', '1f', '1i', '1i' ]
);
if ( !prg ) { return; }

// テクスチャのロード
let textureNumLenna = 1;
gl3.create_texture_canvas( WE.images[ 'lenna.png' ], textureNumLenna );

// フレームバッファを定義
let textureNumFramebuffer = 0;
let framebuffer = gl3.create_framebuffer( canvasWidth, canvasHeight, textureNumFramebuffer );
...
let update = () => {
    ...
    prg.push_shader( [ resolution, mousePosition, nowTime, 1, textureNumLenna ] );
    ...
    prg.set_program();
    prg.set_attribute( quadVbo );
    prg.push_shader( [ resolution, mousePosition, nowTime, 0, textureNumFramebuffer ] );
    gl3.draw_arrays( gl.TRIANGLE_STRIP, 4 );
