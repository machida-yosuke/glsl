...
// 文字の距離関数
float distFuncWord( vec3 _p, float _z ) {
    return max(
        distTexToDist( texture2D( texture, vInvert( saturate( _p.xy + 0.5 ) ) ) ) * 0.5,
        abs( _p.z ) - _z
    );
}
...
float distFunc( vec3 _p ) {
    vec3 p = _p;
    // p.zx = rotate2D( time * PI ) * p.zx;
    // p.yz = rotate2D( time * PI / 0.7 ) * p.yz;
    // p.zx = rotate2D( p.y * 4.0 ) * p.zx;
    // p.yz = rotate2D( p.x * 3.0 ) * p.yz;
    return distFuncWord( p, 0.1 );
}
