...
// JavaScript から送られてくるマウス座標
uniform vec2 mouse;
// JavaScript から送られてくる時間 @@@
uniform float time;
// フラグメントシェーダに渡す色
varying vec3 vColor;
...
void main(){
    ...
    float wave = mouse.x + 1.0; // 0.0 < wave < 2.0

    // マウスカーソル Y 値が強さに影響するように
    float height = mouse.y;

    // 係数からサイン波の値を得る
    float s = sin(vertex.x * 2.0 * PI * wave + time) * height;

    // 係数が色に影響するように
    vColor = vec3(1.0, height, wave);
