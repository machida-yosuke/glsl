...
(() => {
    ...
    window.addEventListener('load', () => {
        ...
        createTexture('./image/sample.jpg', (texture) => {
    ...
    function init(texture){
        ...
        scenePrg.attLocation[1] = gl.getAttribLocation(scenePrg.program, 'positionOffset');
        scenePrg.attLocation[2] = gl.getAttribLocation(scenePrg.program, 'texCoord');
        scenePrg.attStride[0]   = 3;
        scenePrg.attStride[1]   = 3;
        ...
        scenePrg.uniLocation[3] = gl.getUniformLocation(scenePrg.program, 'time');
        scenePrg.uniType[0]     = 'uniformMatrix4fv';
        scenePrg.uniType[1]     = 'uniform4fv';
        scenePrg.uniType[2]     = 'uniform1i';
        scenePrg.uniType[3]     = 'uniform1f';
        ...
        let indexCount = 0;
        {
            ...
            let interval = 0.5;           // ポリゴン同士の間隔
            let hi = interval / 2.0;      // 間隔の半分
            let count = width / interval; // 幅と間隔からループカウント数を求める
            let size = 1.0 / (count + 1); // ポリゴンひとつあたりのテクスチャ座標の幅
            // ループで一気に頂点を定義する
            for(let i = 0; i < count; ++i){
                ...
                for(let j = 0; j < count; ++j){
                    ...
                    z += j * size * 0.1;
                    // 乱数を生成(0.25 ~ 1.0)
                    let r = (Math.floor(Math.random() * 4) + 1) / 4;
                    position.push(
                        -hi,  hi, r, // x, y, 乱数
                         hi,  hi, r, // x, y, 乱数
                        -hi, -hi, r, // x, y, 乱数
        ...
        function render(){
            ...
            gl[scenePrg.uniType[3]](scenePrg.uniLocation[3], nowTime);
            // 描画
