...
(() => {
    ...
    function init(texture){
        ...
        // 頂点カラーを定義する @@@
        let color = [
            1.0, 1.0, 1.0, 0.2, // 1 つ目の頂点の R, G, B, A
            1.0, 1.0, 1.0, 0.3, // 2 つ目の頂点の R, G, B, A
            1.0, 1.0, 1.0, 0.4, // 3 つ目の頂点の R, G, B, A
            1.0, 1.0, 1.0, 0.1  // 4 つ目の頂点の R, G, B, A
        ];
        ...
        // WebGL API 関連の初期設定
        gl.clearColor(0.0, 0.0, 0.0, 1.0); // クリアする色は黒
        gl.clearDepth(1.0);                // クリアする深度
        gl.disable(gl.DEPTH_TEST);         // 深度テストは有効化しない
        ...
        function render(){
            // 描画開始からの経過時間（秒単位）
            nowTime = 10.0 + (Date.now() - startTime) / 1000;
            ...
            // 複数の板ポリゴンを敷き詰めるように描画 @@@
            const LOOP_COUNT = 5;
            const POLY_WIDTH = 1.25;
            const OFFSET = (LOOP_COUNT * POLY_WIDTH) / 2.0;
            for(let i = 0; i < LOOP_COUNT; ++i){
                let x = -OFFSET + POLY_WIDTH * 0.5 + POLY_WIDTH * i;
                for(let j = 0; j < LOOP_COUNT; ++j){
                    let y = -OFFSET + POLY_WIDTH * 0.5 + POLY_WIDTH * j;
                    mat.identity(mMatrix);
                    mat.translate(mMatrix, [x, y, 0.0], mMatrix);
                    mat.rotate(mMatrix, nowTime * 0.1 * (i + j), [0.1, 0.3, 0.8], mMatrix);
